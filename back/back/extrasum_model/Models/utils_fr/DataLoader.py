...
class DataLoader():
    ...
    def merge(self, batch):
        ...
        if self.ner:
            labels_sum = [lbl for e in batch for lbl in e["labels_sum"]]
            labels_ner = [lbl for e in batch for lbl in e["labels_ner"]]
            return {"idx": idxs, "doc_lens": doc_lens, "docs": docs, "labels_sum": labels_sum, "labels_ner": labels_ner}
